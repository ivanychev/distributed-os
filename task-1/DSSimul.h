...
// ATTN context
    ...
    struct context_bully_s {
        ...
        bool isInElection() const {
            return state == State::kInElection;
        }

        bool isWaitingForWinner() const {
            return state == State::kWaitForWinner;
        }

        bool ElectionTimeRanOut(int timediff) const {
            return time - election_start > timediff;
        }

        bool WaitTimeRanOut(int timediff) const {
            return time - wait_start > timediff;
        }

        void SetInElection() {
            state = State::kInElection;
            election_start = time;
        }
    };
